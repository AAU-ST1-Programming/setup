/*
  I modified the Sprite_scroll example:
  - Change delay to 10ms
  - instead of an up/down bar, I am instead using a PPG
...
void loop() {
  ...
  delay(10);  // wait so things do not scroll too fast
  ...
  tcount--;
  if (tcount <= 0) {  // If we have scrolled 40 pixels the redraw text
    tcount = 40;
